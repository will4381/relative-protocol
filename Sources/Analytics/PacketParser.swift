...
public enum PacketParser {
    ...
    /// Parses one raw IP packet into structured metadata.
    /// - Parameters:
    ///   - data: Raw packet bytes as received from tunnel or bridge.
    ///   - ipVersionHint: Optional family hint (`AF_INET`/`AF_INET6`) when version nibble is unavailable.
    /// - Returns: Parsed metadata, or `nil` when packet is unsupported/invalid.
    ...
    /// Computes the QUIC header-protection mask block with the platform AES implementation.
    /// CommonCrypto is hardware-accelerated and FIPS-validated; the inputs here are derived from
    /// wire-visible values (RFC 9001 Initial salts and the destination connection ID), and a wrong
    /// mask fails closed downstream because AEAD authentication rejects the payload.
    /// Verified against the RFC 9001 Appendix A.2 known-answer vector in `PacketParserQuicInitialTests`.
    private static func aes128EncryptBlock(key: Data, block: Data) -> Data? {
        guard key.count == kCCKeySizeAES128, block.count == kCCBlockSizeAES128 else { return nil }
        ...
        guard status == kCCSuccess, movedBytes == kCCBlockSizeAES128 else { return nil }
        return Data(output)
    }
