...
//
//  Bridges `NEPacketTunnelProvider` packet flow into the Leaf core. The
//  adapter owns the read loop, metrics accounting, and hook invocation logic
//  while delegating actual packet processing to the Rust FFI engine.
//
...
/// Minimal interface adopted by both the real Rust FFI bridge and the noop
/// stub. Keeps the adapter decoupled from generated bindings.
protocol LeafRuntimeEngine {
...
/// Shim used when the Rust FFI-generated bindings are unavailable. It simply
/// reflects packets back through the tunnel so the provider remains responsive.
final class NoOpLeafRuntimeEngine: LeafRuntimeEngine, @unchecked Sendable {
...
final class LeafAdapter: @unchecked Sendable {
    ...
    private let ioQueue = DispatchQueue(label: "RelativeProtocolTunnel.LeafAdapter", qos: .userInitiated)
    ...
    ///   - engine: Concrete engine implementation (Rust FFI or noop).
    ...
    func start() throws {
        ...
        let callbacks = LeafRuntimeCallbacks(
    ...
    private func handleReadResult(packets: [Data], protocols: [NSNumber], channel: AsyncChannel<PacketBatch>) {
        ...
        let totalBytes = LeafAdapter.totalBytes(in: packets)
    ...
    private func processOutbound(_ batch: PacketBatch, handler: @escaping @Sendable (_ packets: [Data], _ protocols: [NSNumber]) -> Void) async {
        ...
        for (index, packet) in batch.packets.enumerated() {
            ...
            if LeafAdapter.shouldAnalyze(packet: packet, direction: .outbound) {
    ...
    private func processInbound(_ batch: PacketBatch) async {
        ...
        for (index, packet) in batch.packets.enumerated() {
            ...
            if LeafAdapter.shouldAnalyze(packet: packet, direction: .inbound) {
                trafficAnalyzer?.ingest(sample: .init(
                    direction: .inbound,
                    payload: packet,
                    protocolNumber: proto
