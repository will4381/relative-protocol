...
open class RelativePacketTunnelProvider: NEPacketTunnelProvider {
            ...
            self.queue.async {
                ...
                case .success(let port):
                    ...
                    do {
                        ...
                        bridge.startReadLoop { [weak self] packet, family in
                            self?.handleInboundPacket(packet, family: family)
                        }
    ...
    private func handleInboundPacket(_ packet: Data, family: Int32) {
        guard !isStopping else { return }
        let proto = NSNumber(value: family)
        let success = packetFlow.writePackets([packet], withProtocols: [proto])
        if !success {
            logger.error("Failed to write inbound packet to flow.")
        }

        let timestamp = Date().timeIntervalSince1970
        let packetCopy = packet
        metricsQueue.async { [weak self] in
            self?.recordInboundPacket(packetCopy, family: family, timestamp: timestamp)
        }
    }
    ...
    private func recordOutboundPackets(_ packets: [Data], protocols: [NSNumber], timestamp: TimeInterval) {
        ...
        for (index, packet) in packets.enumerated() {
            ...
            if let metadata = PacketParser.parse(packet, ipVersionHint: protoHint) {
                ...
                if let dnsQuery = metadata.dnsQueryName {
    ...
    private func recordInboundPacket(_ packet: Data, family: Int32, timestamp: TimeInterval) {
        guard let metricsBuffer else { return }
